package dev.eblanvisuals.client.ui.hud;
...
import dev.eblanvisuals.modules.settings.impl.PositionSetting;
...
import static dev.eblanvisuals.modules.api.Module.fullNullCheck;
...
public abstract class HudElement extends Module {
    ...
    public void onRender2D(EventRender2D e) {
        ...
        if (button) {
            ...
            dev.eblanvisuals.client.ui.hud.HudElement current = EblanVisuals.getInstance().getHudManager().getCurrentDragging();
            ...
            if (!(mc.currentScreen instanceof ChatScreen)) {
                dragging = false;
                button = false;
                EblanVisuals.getInstance().getHudManager().setCurrentDragging(null);
                return;
            }
            ...
            if (!dragging && MathUtils.isHovered(getX(), getY(), getWidth(), getHeight(), mouseX(), mouseY())) {
                ...
                dragY = mouseY() - getY();
                dragging = true;
                EblanVisuals.getInstance().getHudManager().setCurrentDragging(this);
            }
...
//            e.getContext().drawTextWithShadow(mc.textRenderer, text, x, y, 0xFFFFFF);
    ...
    public void onRender2DX2(EventRender2D e) {
        ...
        Setting<?> setting = EblanVisuals.getInstance().getHudManager().getElements().getName(getName());
    ...
    public void onMouse(EventMouse e) {
        ...
        if (e.getAction() == 0) {
            ...
            if (EblanVisuals.getInstance().getHudManager().getCurrentDragging() == null ||
                EblanVisuals.getInstance().getHudManager().getCurrentDragging() == this) {
                button = true;
            }
            ...
            if (e.getButton() == 1) {
                if (MathUtils.isHovered(getX(), getY(), getWidth(), getHeight(), mouseX(), mouseY())) {
                    ...
                    if (settings.size() == 1) {
                        ...
                        if (EblanVisuals.getInstance().getHudManager().getWindow() != null)
                            EblanVisuals.getInstance().getHudManager().getWindow().reset();
                        for (HudElement element : EblanVisuals.getInstance().getHudManager().getHudElements()) {
